...
test_x = test.iloc[:, 2:]  # 行全部选择，列选择第三列之后的

train_x = []
train_y = []

for i in range(15):  # 15组数据
    x = train.iloc[:, i:i + 9]  # 前九个小时的数据作为train的特征
    # notice if we don't set columns name, it will have different columns name in each iteration
    x.columns = np.array(range(9))
    y = train.iloc[:, i + 9]  # 第十个小时的数据作为train的标签
...
sampleSubmission.to_csv(path + 'result.csv')  # 将第十个小时的预测结果保存到result.csv